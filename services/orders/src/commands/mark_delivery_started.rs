...
impl Command {
    ...
    pub async fn mark_delivery_started(
        &mut self,
        id: Uuid,
        ...
        match order.mark_delivery_started(timestamp) {
            Err(_) => Err(Error::AlreadyDeliveryStarted),
    ...
    async fn get(&self, id: Uuid) -> Result<Order, Error> {
        match self.storage.get(id).await {
            Ok(o) => Ok(o),
            Err(e) => {
                log::warn!("mark_delivery_started: {}", e);
                match e {
